...
from clopa.inference_sessions.pretrained_inference_session import nnInteractiveInferenceSession as PreTrainednnInteractiveSession
from clopa.inference_sessions.adapted_inference_session import nnInteractiveInferenceSession as AdaptednnInteractiveSession
from nnunetv2.utilities.helpers import empty_cache
from clopa.adaptation.data_handling import DataHandler
from clopa.adaptation.adaptation_trigger_criteria import AdaptationTriggerCriterionRegistry
from clopa.adaptation.adaptation_planning import AdaptationPlanner
from clopa.adaptation.adaptation_executor import AdaptationExecutor

CLOPA_ROOT = Path(__file__).resolve().parent.parent
pretrained_ckpt_dir = str(CLOPA_ROOT / 'ckpt' / 'nnInteractive_v1.0')
adaptation_config_registry_path = str(CLOPA_ROOT / 'clopa' / 'adaptation' / 'adaptation_config_registry.txt')
...
class InferApp:
    def __init__(self, 
        ...
        if self.algorithm_state == {} and not self.enable_adaptation and not self.execute_on_adapted:
            ...
            self.experiment_dir = str(CLOPA_ROOT / 'experiment_storage' / experiment_subdir)
            ...
            self.algo_cache_name = algo_cache_name
            experiment_subdir =  self.algo_cache_name
            experiment_dir = str(CLOPA_ROOT / 'experiment_storage' / experiment_subdir)
    ...
    def binary_subject_prep(self, request:dict):
        if self.dataset_level_schema is None:
            ...
            # genuine per-sample channel list rather than echoing the dataset-level value, see
            # IS_Validate/src/front_back_interactor/simulation_orchestrator.py). set(...) works
            # whether task_channels is a list of names (this app's own napari export path) or a
            # dict keyed by name (IS_Validate's run.py pipeline) — both give the same set of
            # channel names either way.
